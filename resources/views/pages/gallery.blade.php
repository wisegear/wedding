...
@section('content')
    <section class="space-y-8">
        <div class="shadow-thistle rounded-[2rem] border border-white/80 bg-white/92 p-8">
            <p class="text-thistle text-xs uppercase tracking-[0.35em]">Gallery</p>
            <h1 class="font-display mt-4 text-5xl text-[#2f2540]">Shared photos from the wedding</h1>
        ...
        @auth
            <div class="rounded-[2rem] border border-[#d9cde6] bg-[#f3edf8] p-6">
                ...
                <form class="mt-4 grid gap-4 md:grid-cols-[1fr_1fr_auto]" method="POST" action="{{ route('gallery.upload') }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input class="block w-full rounded-2xl border border-[#cbb6de] bg-white px-4 py-3 text-sm" id="image" name="image" type="file" accept="image/*" required>
                        @error('image')
                    ...
                    <div>
                        ...
                        <input class="block w-full rounded-2xl border border-[#cbb6de] bg-white px-4 py-3 text-sm" id="caption" name="caption" type="text" maxlength="255" value="{{ old('caption') }}">
                        @error('caption')
                            <p class="mt-2 text-sm text-red-700">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-end">
                        <button class="bg-heather-deep w-full rounded-full px-5 py-3 text-sm font-semibold text-white hover:opacity-95 md:w-auto" type="submit">
                            Upload
                        </button>
                    </div>
                </form>
        ...
        @else
            <div class="rounded-[2rem] border border-[#d7e0e5] bg-[#eef3f5] p-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($uploads as $upload)
                <article class="shadow-thistle overflow-hidden rounded-[1.75rem] border border-white/80 bg-white/92">
