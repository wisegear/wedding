...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-garden-page min-h-screen text-stone-900 antialiased">
        ...
        <div class="absolute inset-x-0 top-0 -z-10 h-[28rem] bg-[radial-gradient(circle_at_top,rgba(239,191,187,0.28),transparent_34%),linear-gradient(180deg,rgba(255,229,228,0.62),rgba(255,229,228,0))]"></div>
        <div class="absolute right-0 top-12 -z-10 h-72 w-72 rounded-full bg-[#efbfbb]/38 blur-3xl"></div>
        ...
        <div class="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-6 lg:px-8">
            <header class="bg-garden-panel shadow-garden flex flex-col gap-4 rounded-[2rem] border border-white/70 px-5 py-4 backdrop-blur lg:flex-row lg:items-center lg:justify-between">
                <a class="flex items-center gap-3" href="{{ route('home') }}">
                    <span class="flex size-11 items-center justify-center rounded-full bg-[#ffe5e4] text-[#4d513f] ring-1 ring-[#666956]/15">
                ...
                <nav class="flex flex-wrap items-center gap-2 text-sm">
                    ...
                    @else
                        <a class="rounded-full bg-[#ffe5e4] px-4 py-2 text-[#4d513f] hover:bg-[#efbfbb]" href="{{ route('register') }}">Register</a>
